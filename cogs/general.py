...
class General(GuildedCog, name="general"):
    ...
    async def botinfo(self, context: Context) -> None:
        ...
        embed.add_field(
            ...
            value='People who join the server will be kicked automatically after 7 days if no mod intervenes. This is achieved through the "visa" role. In order to prevent autokicking, be nice and cool and ask a mod to remove your visa role if you\'ve warmed up to the server.',
        )
        embed.add_field(name="Owner:", value="Bernoulli#2628", inline=True)
        embed.add_field(
            name="Prefix:",
            value=f"/ (Slash Commands) or {self.bot.config.command_prefix} for normal commands",
            inline=False,
        )
    ...
    async def serverinfo(self, context: Context) -> None:
        ...
        embed = discord.Embed(
            title="**Server Name:**", description=description, color=0x9C84EF
        )
