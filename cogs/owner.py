...
class Owner(commands.Cog, name="owner"):
  ...
  async def ownerhelp(self, context: Context) -> None:
    ...
    help_text = "\n".join(data)
    owner_guilded = ['designate_spam_channel', 'designate_status_channel']
    name = "visabot"
    cog = self.bot.get_cog(name)
    commands = cog.get_commands()
    for command in commands:
  ...
  async def load(self, context: Context, cog: str) -> None:
    ...
    try:
      ...
      embed = discord.Embed(description=f"Could not load the `{cog}` cog.",
                            color=0xE02B2B)
      await context.send(embed=embed)
      return
    embed = discord.Embed(description=f"Successfully loaded the `{cog}` cog.",
                          color=0x9C84EF)
    await context.send(embed=embed)
  ...
  async def unload(self, context: Context, cog: str) -> None:
    ...
    try:
      ...
      embed = discord.Embed(description=f"Could not unload the `{cog}` cog.",
                            color=0xE02B2B)
      await context.send(embed=embed)
      return
    embed = discord.Embed(
  ...
  async def reload(self, context: Context, cog: str) -> None:
    ...
    try:
      ...
      embed = discord.Embed(description=f"Could not reload the `{cog}` cog.",
                            color=0xE02B2B)
      await context.send(embed=embed)
      return
    embed = discord.Embed(
